...
class VoiceFile:
	...
	def loop_file(self, filename):
		...
		if(len(basename_split) < 2): return
		...
		for line in lines:
			if line[:5] == b'_VPLY':
				...
				if(voices[1] in self.voice_filename): continue
				self.voice_filename[voices[1]] = [voices[0], routelist[basename_split[0]], basename_split[1]]
		fs.close()
